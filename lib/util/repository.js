...
module.exports = {
  ...
  installPackage(packageName, version) {
    ...
    if (installedVersion === undefined || (version && !semver.satisfies(installedVersion, version))) {
      ...
      const pkgs = {};
      if (packageName === 'yeoman-environment' && version && semver.lt(version, '2.9.0')) {
  ...
  installPackages(packages) {
    ...
    const packagesArgs = Object.entries(packages).map(([key, value]) => value ? `${key}@${value}` : key);
