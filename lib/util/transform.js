    ...
    yoResolveFiles
      ...
      .find(yoResolve =>
        Object.entries(yoResolve).some(([pattern, status]) => {
          if (minimatch(slash(filePath), pattern)) {
            fileStatus = status;
            return true;
          }
          return false;
...
export function patternFilter(pattern, options) {
  ...
  return filter(file => minimatch.match(slash(file.path)));
}
...
export function patternSpy(spy, pattern, options) {
  ...
  return passthrough(file => (minimatch.match(slash(file.path)) ? spy(file) : undefined));
}
