...
class Conflicter {
  ...
  createChecker() {
    ...
    return through.obj(function (file, enc, cb) {
      ...
      self.checkForCollision(file, (err, status) => {
        if (err) {
          cb(err);
          return;
        }
  ...
  resolve(cb) {
    ...
    async.series(this.conflicts.map(conflict => resolveConflicts(conflict)), cb.bind(this));
  }
  ...
  collision(file, cb) {
    ...
    if (!fs.existsSync(file.path)) {
      ...
      if (this.bail) {
        ...
        throw ConflicterConflictError.create('Process aborted by conflict');
      }
