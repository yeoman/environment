...
composability.prepareEnvironment = async function (namespaces) {
  ...
  const addPeerGenerators = async (packageName, packageRange = '*') => {
    ...
    const {peerDependencies} = packages.versions[version];
    if (peerDependencies) {
...
class YeomanWith {
  ...
  call(methods, ...methodArgs) {
    ...
    this.generatorApis.forEach(generatorApi => runMethods(generatorApi));
    return this;
  }
}
...
class YeomanCompose {
  ...
  async with(namespace, generatorOptions, ...methodArgs) {
    ...
    const namespacesId = namespace.instanceId && namespace.instanceId === '*' ? this._getInstanceNames(namespace.namespace).map(instanceId => {
