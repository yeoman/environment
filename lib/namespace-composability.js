...
class YeomanCompose {
   ...
   *
   * @param {String|YeomanNamespace} namespace - Namespace
   * @param {...Object} [methodArgs] - Arguments to be passed to the method
   * @return {Promise} Promise any.
   */
  async call(namespace, ...methodArgs) {
    debug(`Calling ${namespace} at ${this._destinationRoot} with arguments '${methodArgs}'`);
    namespace = this.env.requireNamespace(namespace);
  ...
  async with(namespace, generatorOptions, ...methodArgs) {
    ...
    if (namespace.instanceId && namespace.instanceId === '*') {
      return Promise.all(this._getInstanceNames(namespace.namespace).map(instanceId => {
        return this.with(namespace.with({instanceId}), generatorOptions, ...methodArgs);
      }));
    }
    ...
    const callMethods = namespace => {
      ...
      return this.call(namespace, ...methodArgs);
    };
