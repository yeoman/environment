...
Environment.prototype.create = function create(namespace, options) {
  ...
  if (!_.isFunction(Generator)) {
    return this.error(
      new Error(
        chalk.red('You don\'t seem to have a generator with the name ' + chalk.bold(namespace) + ' installed.') + '\n' +
        'But help is on the way:\n\n' +
        'You can see available generators via ' +
        '`' + chalk.bold('npm search yeoman-generator') + '` or via ' + chalk.bold('http://yeoman.io/generators/') + '. \n' +
        'Install them with `' + chalk.bold('npm install [generator-name]') + '`.\n\n' +
        'To see all your installed generators run `' + chalk.bold('yo') + '` without any arguments. ' +
        'Adding the `' + chalk.bold('--help') + '` option will also show subgenerators. \n\n' +
        'Run ' + chalk.bold('`yo doctor`') + ' if you encounter problems with running installed generators.'
      )
    );
  }
