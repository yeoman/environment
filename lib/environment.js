...
class Environment extends Base {
   ...
   *
   * @return {Promise} Promise a Environment instance
   */
  static prepareCommand(GeneratorClass) {
    const command = Base.addEnvironmentOptions(new YeomanCommand());
  ...
  instantiate(Generator, args, options) {
    ...
    let namespaceId = this.toNamespace(namespace);
    if (namespaceId) {
      if (args.length === 1) {
        namespaceId = namespaceId.with({instanceId: args[0]});
      }
  ...
  runGenerator(generator) {
    if (generator.runWithOptions || generator.queueTasks) {
      /*
         * Yeoman-generator >= 4.8.0, just queue the tasks.
         * It should emit errors to environment directly.
         */
      this.queueGenerator(generator);
    } else {
      this.compatibilityMode = 'v4';
      ...
      /*
         ...
         */
      generator.on('error', error => this.emit('generator:reject', error));
      ...
      /*
         ...
         */
      const generatorCallback = error => {
        return error === undefined ? () => this.emit('generator:resolve') : this.emit('generator:reject', error);
      };

      const genPromise = generator.run(generatorCallback);
      if (genPromise instanceof Promise) {
        genPromise.then(() => this.emit('generator:resolve'), error => this.emit('generator:reject', error));
      }
    }
  ...
  commitSharedFs(stream = this.sharedFs.stream()) {
    return new Promise((resolve, reject) => {
      ...
      const transformStreams = [
        ...
        createYoRcTransform(),
        pipe(file => this.conflicter.checkForCollision(file), {ordered: false, maxParallel: 10}),
        createConflicterStatusTransform()
      ];
