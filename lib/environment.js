...
class Environment extends EventEmitter {
  ...
  create(namespace, options) {
    ...
    if (typeof Generator !== 'function') {
      const scopedNameRegex = new RegExp('@[^/]*/[^/]*'); // Check if namespace matches "@foo/bar" pattern
      let generatorHint = '';
      if (namespace.match(scopedNameRegex)) {
        const prefix = namespace.match(new RegExp('@[^/]*'));
        const generatorName = namespace.match(new RegExp('[^/]*$'));
