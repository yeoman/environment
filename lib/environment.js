...
class Environment extends Base {
   ...
   */
  queuePackageManagerInstall(packageJsonAbsolutePath) {
    const suffix = packageJsonAbsolutePath ? ` at ${packageJsonAbsolutePath}` : '';
    this.runLoop.add(
      'install',
      (done, stop) => this.packageManagerInstallTask(packageJsonAbsolutePath).then(done, stop),
      {once: `package manager install${suffix}`}
    );
  }
}
