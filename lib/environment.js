...
class Environment extends EventEmitter {
  ...
  register(name, namespace, packagePath) {
    ...
    this.store.add(namespace, modulePath, Environment.UNKNOWN_RESOLVED, packagePath);
   ...
   */
  installLocalGenerators(packages) {
    const entries = Object.entries(packages).filter(([packageName, version]) => !Environment.repository.verifyInstalledVersion(packageName, version));
    if (entries.length === 0) {
      return true;
    }
    const toInstall = {};
    ...
    if (Environment.repository.installPackages(toInstall)) {
   ...
   */
  lookupLocalPackages(packagesToLookup = 'generator-*') {
    this.lookup({packagePatterns: packagesToLookup, npmPaths: Environment.repository.nodeModulesPath});
  }
 ...
 */
Environment.repository = require('./util/repository');
