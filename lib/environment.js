...
class Environment extends EventEmitter {
  ...
  constructor(args, opts, adapter) {
    ...
    if (this.options.yeomanRepository) {
      this.repository.repositoryPath = this.options.yeomanRepository;
    }

    experimental = this.options.experimental === true;

    if (experimental) {
      Object.assign(this, require('./namespace'));
      Object.assign(this, require('./namespace-composability'));
      Object.assign(this, require('./generators'));
    }
   ...
   * @param {String} namespaceOrPath
   * @param {Object} options
   * @param {boolean} [skipRegister=false] Skip registering the generator instance.
   */
   ...
   * @param {boolean}      [options.skipRegister=false] Skip registering the generator instance.
   */
  instantiate(Generator, options) {
    ...
    let genNamespaceId;
    if (!options.skipRegister && this.toNamespace && this.nextInstanceId) {
      const namespace = this.toNamespace(options.namespaceId || Generator.namespace || Environment.UNKNOWN_NAMESPACE);
      if (namespace) {
        genNamespaceId = this.nextInstanceId(namespace);
      }
    }
    ...
    const environmentOptions = {
      ...
      namespaceId: genNamespaceId ? genNamespaceId.id : options.namespaceId
    };
    ...
    if (this.registerInstance && !options.skipRegister && genNamespaceId) {
      this.registerInstance(genNamespaceId, generator);
    }
    return generator;
  }
  ...
  runGenerator(generator, done) {
    ...
    if (this.registerInstance) {
      this.registerInstance(generator);
    }
