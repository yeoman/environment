...
class Environment extends EventEmitter {
  ...
  static lookupGenerator(namespace, options = {singleResult: true}) {
    ...
    options.filePatterns = options.filePatterns || Environment.lookups.map(prefix => path.join(prefix, '*/index.js'));
    ...
    resolver.packageLookup.sync(options, module => {
      ...
      if (namespace === fileNS || (options.packagePath && namespace === Environment.namespaceToName(fileNS))) {
        ...
        const returnPath = options.packagePath ? module.packagePath : (options.generatorPath ? path.posix.dirname(path.posix.dirname(filename)) : filename);
