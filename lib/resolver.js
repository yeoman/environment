  ...
  for (const root of searchPaths) {
    ...
    try {
      modules = modules.concat(globby.sync(
        ...
        {cwd: root, onlyDirectories: true, expandDirectories: false, absolute: true, deep: 0}
      ));
      ...
      for (const scope of scopes) {
        modules = modules.concat(globby.sync(
          options.packagePatterns,
          {cwd: scope, onlyDirectories: true, expandDirectories: false, absolute: true, deep: 0}
        ));
      }
    } catch (error) {
