...
resolver.lookup = function (options = {localOnly: false}, cb = undefined) {
  ...
  this.moduleFinder.sync(options, module => {
    ...
    if (! this._tryRegistering(generatorPath, packagePath, namespace)) {
      return false;
    }
    generators.push({generatorPath, packagePath, namespace});
...
moduleFinder.sync = function (options = {}, find = module => module) {
  ...
  for (const packagePath of options.packagePaths) {
    for (const filePath of globby.sync(options.filePatterns, {cwd: packagePath, absolute: true, deep: 1})) {
