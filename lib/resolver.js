...
resolver.lookup = function (cb) {
  cb = cb || _.noop;
  const globPatterns = [];
  const modulePaths = this.getNpmPaths().reverse();
  const packagePatterns = ['generator-*', '\\@*/generator-*'];
  for (const root of modulePaths) {
    for (const packageRoot of packagePatterns) {
      for (const lookup of this.lookups) {
        globPatterns.push(`${root}/${packageRoot}/${lookup}/*/index.js`);
      }
    }
  }

  fg(globPatterns, {absolute: true, onlyFiles: true, extension: false}).then(files => {
    console.log(files);
...
resolver.getNpmPaths = function () {
  ...
  return _.uniq(paths.reverse()).filter(searchPath => searchPath !== '/Users');
};
