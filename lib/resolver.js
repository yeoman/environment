...
resolver.moduleFinder = moduleFinder;
...
resolver.lookup = function (options = {localOnly: false}, cb = undefined) {
  ...
  options.filePatterns = options.filePatterns || lookups.map(prefix => path.join(prefix, '*/index.js'));
  ...
  const generators = this.moduleFinder.sync(options, module => {
    module.namespace = this.namespace(module.filename);
    module.registered = this._tryRegistering(module.filename, module.packagePath, module.namespace);
    ...
    return module;
  }).filter(module => module.registered);
 ...
 * @param {function}     [found] - Criteria to finish lookup.
 */
moduleFinder.sync = function (options = {}, found = module => module) {
  ...
  debug('Lookup options calculated options: %o', options);

  const modules = [];
  for (const packagePath of options.packagePaths) {
    for (const filename of globby.sync(options.filePatterns, {cwd: packagePath, absolute: true, deep: 1})) {
      const module = found({filename, packagePath});
      if (!module) {
        return;
      }
      if (module.found) {
...
resolver.findGeneratorsIn = function (searchPaths, options = {}) {
  console.warn('findGeneratorsIn has been deprecated');
  ...
  return this.moduleFinder.findPackagesIn(searchPaths, options);
};
 ...
 * @return {Array} List of the generator modules path
 */
moduleFinder.findPackagesIn = function (searchPaths, options = {}) {
...
resolver.getNpmPaths = function (options = {}) {
  console.warn('getNpmPaths has been deprecated');
  ...
  return this.moduleFinder.getNpmPaths(options);
};
 ...
 * @return {Array} lookup paths
 */
moduleFinder.getNpmPaths = function (options = {}) {
  // Resolve signature where options is boolean (localOnly).
  if (typeof options === 'boolean') {
    options = {localOnly: options};
  }
 ...
 * @return {Array} lookup paths
 */
moduleFinder._getLocalNpmPaths = function () {
  const paths = [];
 ...
 * @return {Array} lookup paths
 */
moduleFinder._getGlobalNpmPaths = function (filterPaths = false) {
  let paths = [];
