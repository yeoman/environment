...
class YeomanCompose extends EventEmitter {
  ...
  async with(namespace, generatorOptions, ...methodArgs) {
    ...
    namespaces = namespaces.map(namespace => requireNamespace(namespace));
    ...
    const namespacesId = namespaces.flatMap(namespace =>
      namespace.instanceId && namespace.instanceId === '*'
        ? this._getInstanceNames(namespace.namespace).map(instanceId => namespace.with({ instanceId }).id)
        : [namespace.id],
    );
   ...
   */
  async _load(namespace, generatorOptions) {
   ...
   */
  async _composeWithGenerator(namespace, generatorOptions) {
   ...
   */
  async _queue(namespace, generatorOptions) {
